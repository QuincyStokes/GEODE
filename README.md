<!--
  HOW TO USE THIS:
  Replace the README.md in your GEODE repo with this. Also, in the repo's
  "About" panel (gear icon, top-right of the repo page):
    - Description: "Solo-shipped co-op survival + tower-defense game. Unity, C#, online multiplayer for up to 12 players."
    - Website: your Steam page URL
    - Topics: unity, csharp, game-development, multiplayer, indie-game, steam
  Add a screenshot or short GIF near the top — a visual is the single biggest
  upgrade you can make to this page. Drop it in the repo and reference it below.
-->

# GEODE

**An open-world co-op survival + tower-defense game, solo-developed in Unity.**
*Art by Annika Stokes.*

**[Now on Steam — Free to Play](https://store.steampowered.com/app/4213520/GEODE/)** · 100% positive player rating

![GEODE gameplay](MainCapsule.png)  

## About
GEODE takes place in a strangely corrupt world, where meteors that landed thousands of years ago are now eroding away, revealing strange crystals and minerals. These crystals warp the environment around them. Explore, build defenses, and survive with friends for as long as you can.

## Features
- **Online co-op multiplayer** for up to 12 players
- **Survival meets tower defense** in an open world
- A corrupt, crystal-warped world to explore

## Tech
- **Engine / Language:** Unity, C#
- **Networking:** Unity Netcode for GameObjects + peer-to-peer, with client state synchronization
- **Architecture:** rebuilt from the ground up around SOLID principles and OOP class hierarchies for scalability
- **Performance:** profiled and optimized with the Unity Profiler

## Links
[Steam](https://store.steampowered.com/) · [quincystokes.com](https://www.quincystokes.com/) · [LinkedIn](https://www.linkedin.com/in/quincy-stokes)
